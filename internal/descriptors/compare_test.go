...
func TestCompareLevelsReturnsCompleteSummariesBeforeLimit(t *testing.T) {
	got, err := descriptors.CompareLevels(context.Background(), queryDataset(), descriptors.CompareLevelsInput{
		Corpus: "cefr",
		Scale:  "turntaking",
	...
	if got.Kind != "descriptor_level_comparison" || got.SchemaVersion != descriptors.SchemaVersion ||
		got.Total != 2 || got.Returned != 1 {
		t.Errorf(
			"descriptors.CompareLevels() metadata = kind %q schema %q total %d returned %d, want %s %s 2 1",
			got.Kind,
			got.SchemaVersion,
			got.Total,
			got.Returned,
			"descriptor_level_comparison",
	...
	wantSummaries := []descriptors.LevelSummary{
		{Level: "a1", Total: 1, Present: true},
		{Level: "a2", Total: 0, Present: false},
		{Level: "b1", Total: 1, Present: true},
	}
