...
func TestGetReturnsNotFoundAndPreservesIDCaseSemantics(t *testing.T) {
	...
	_, err = descriptors.Get(context.Background(), queryDataset(), descriptors.GetInput{Corpus: "cefr", ID: "CEFR.PRODUCTION.SPEAKING.DESCRIPTORS.ADDRESSING_AUDIENCES.DELIVER_TOAST.A1"})
	_ = requireCodedError(t, err, "not_found")

	_, err = descriptors.Get(context.Background(), queryDataset(), descriptors.GetInput{Corpus: "cefr", Code: "does_not_exist"})
	_ = requireCodedError(t, err, "not_found")
}
