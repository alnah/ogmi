...
func TestQueryMatchesTextAcrossDescriptorFields(t *testing.T) {
	tests := []struct {
		...
		{name: "description", query: "short toast", wantIDs: []string{"cefr.production.speaking.descriptors.addressing_audiences.deliver_toast.a1"}, wantTotal: 1},
		{name: "id", query: "present_simple_announcement.a2", wantIDs: []string{"cefr.production.speaking.descriptors.addressing_audiences.present_simple_announcement.a2"}, wantTotal: 1},
		{name: "path", query: "production", wantIDs: []string{
			"cefr.production.speaking.descriptors.addressing_audiences.deliver_toast.a1",
			"cefr.production.speaking.descriptors.addressing_audiences.present_simple_announcement.a2",
			"cefr.production.speaking.descriptors.turntaking.open_simple_exchange.a1",
			"cefr.production.speaking.descriptors.turntaking.maintain_exchange.b1",
		}, wantTotal: 4},
	}
...
func TestQueryPaginationNormalizesOffsets(t *testing.T) {
	tests := []struct {
		...
		{name: "negative", offset: -3, wantResultOffs: 0, wantIDs: []string{"cefr.production.speaking.descriptors.addressing_audiences.deliver_toast.a1"}},
		{name: "beyond length", offset: 99, wantResultOffs: 99, wantIDs: []string{}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := requireQuery(t, descriptors.Filters{Corpora: []string{"cefr"}, SortBy: descriptors.FieldID, Offset: tt.offset, Limit: 1})
